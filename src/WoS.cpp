...
Eigen::Vector3d uniform_ball_sampling() { return uniform_sphere_sampling() * sqrt(random(0, 1)); }
...
//       if (steps == 0) {
//         first_direction = new_direction.normalized(); //the normalize is unnecessary since the direction is always
//         normalized. first_R = R; grad = sphere_volume(R) * f(y) * lapdg3d(x,y,R);
//       }
...
walk_on_sphere_single_point(const std::function<std::pair<double, double>(const Eigen::Vector3d)> &sdf_bc,
...
void walk_on_spheres(const std::function<std::pair<double, double>(const Eigen::Vector3d)> &sdf_bc,
  ...
  igl::parallel_for(P.rows(), [&](int i) {
    double val;
    Eigen::Vector3d grad;
    std::tie(val, grad) = walk_on_sphere_single_point(sdf_bc, f, P.row(i), num_walks);
    U[i] = val;
    U_grad.row(i) = grad;
  });
}